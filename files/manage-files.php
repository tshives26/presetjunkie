...
$page_title = __("Manage preset files", "cftp_admin");
  ...
  if (!empty($_POST["batch"])) {
    ...
    switch ($_POST["action"]) {
      case "hide":
        ...
        $flash->success(
          __("The selected files were marked as hidden.", "cftp_admin")
        );
        break;
      case "show":
        foreach ($selected_files as $file_id) {
        ...
        $flash->success(
          __("The selected files were marked as visible.", "cftp_admin")
        );
        break;
      case "hide_everyone":
        foreach ($selected_files as $file_id) {
      ...
      case "unassign":
        ...
        $flash->success(
          __("The selected files were successfully unassigned.", "cftp_admin")
        );
        break;
      case "delete":
        $delete_results = [
          ...
          <form action="manage-files.php" name="files_filters" method="get" class="form-inline form_filters">
            ...
            <div class="form-group group_float">
              <select name="uploader" id="uploader" class="txtfield form-control">
                <?php foreach ($filter_options_uploader
                  as $val => $text) { ?>
            ...
            <div class="form-group group_float">
              <select name="assigned" id="assigned" class="txtfield form-control">
                <?php foreach ($filter_options_assigned
                  as $val => $text) { ?>
                  <option value="<?php echo $val; ?>" <?php if (
                                                        isset($_GET["assigned"]) &&
                                                        $_GET["assigned"] == $val
            ...
            <button type="submit" id="btn_proceed_filter_clients" class="btn btn-sm btn-default"><?php _e(
                                                                                                    "Filter",
                                                                                                    "cftp_admin"
                                                                                                  ); ?></button>
          </form>
            ...
            <div class="form-group group_float">
              ...
              <select name="action" id="action" class="txtfield form-control">
                ...
                if (CURRENT_USER_LEVEL != "0") {
                  $actions_options["zip"] = __(
                    "Download zipped",
                    "cftp_admin"
                  );
                  if (!isset($search_on)) {
                    $actions_options["hide_everyone"] = __(
                      "Set to hidden from everyone already assigned",
                      "cftp_admin"
                    );
                    $actions_options["show_everyone"] = __(
                ...
                if (
                  ...
                  $actions_options["hide"] = __(
                    "Set to hidden",
                    "cftp_admin"
                  );
                  $actions_options["show"] = __(
                    "Set to visible",
                    "cftp_admin"
                  );
                  $actions_options["unassign"] = __(
                    "Unassign",
                    "cftp_admin"
                  );
                } else {
      ...
      if (!$count) {
        if (isset($no_results_error)) {
          switch ($no_results_error) {
            ...
            case "none_assigned":
              $no_results_message = __(
                "There are no files assigned to this client.",
                "cftp_admin"
              );
              break;
            case "account_level":
          ...
          $no_results_message = __(
            "There are no files for this client.",
            "cftp_admin"
          );
        }
        ...
        while ($row = $sql->fetch()) {
          ...
          if ($file->isPublic()) {
            ...
            if (get_option("enable_landing_for_all_files") == "1") {
              $visibility_link =
                '<a href="javascript:void(0);" class="btn btn-default btn-sm public_link" data-type="file" data-public-url="' .
                $file->public_url .
                '">' .
              ...
              $visibility_link =
                '<a href="javascript:void(0);" class="btn btn-default btn-sm disabled" title="">' .
                __("None", "cftp_admin") .
                "</a>";
            }
          }
          ...
          if ($file->expires == "0") {
            ...
            if ($file->expired == true) {
              ...
              $expires_label =
                __("Expired on", "cftp_admin") . " " . $expires_date;
            } else {
              $expires_button = "info";
              $expires_label =
                __("Expires on", "cftp_admin") . " " . $expires_date;
            }
          }
          ...
          if (isset($search_on)) {
            ...
            $status_label =
              $hidden == 1
              ? __("Hidden", "cftp_admin")
              : __("Visible", "cftp_admin");
          }

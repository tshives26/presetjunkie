        ...
        switch ($bfstatus["status"]) {
          case "delay":
            if (is_numeric($bfstatus["message"])) {
              $flash->error(
                '<div id="message_countdown">' .
                  sprintf(
                    __(
                      "Please wait %s seconds before attempting to log in again.",
                      "cftp_admin"
                    ),
                    '<span class="seconds_countdown">' .
                      $bfstatus["message"] .
                      "</span>"
                  ) .
                  "</div>"
              ...
              if ($bfstatus["message"] > 150) {
                $flash->error(
                  sprintf(
                    __(
                      "Warning: You are about to reach the failed attempts limit, which will completely block your access for a few minutes.",
                      "cftp_admin"
                    ),
                    $bfstatus["message"]
                  )
            ...
            <div class="tab-content">
                <div role="tabpanel" class="tab-pane fade in active" id="local">
                    ...
                    <div class="login_form_links">
                        <?php _e(
                          "<b>Login required to view/download/upload. No email verification necessary for registration. Login used to increase security while preventing bots and malicious files.</b>",
                          "cftp_admin"
                        ); ?>
                        <p id="reset_pass_link"><?php _e(
                          "Forgot your password?",
                          "cftp_admin"
                        ); ?> <a href="<?php echo BASE_URI; ?>reset-password.php"><?php _e(
                                "Set up a new one.",
                                "cftp_admin"
                                ); ?></a></p>
                        <?php if (
                          get_option("clients_can_register") == "1"
                        ) { ?>
                                <p id="register_link"><?php _e(
                                  "Don't have an account yet?",
                                  "cftp_admin"
                                ); ?> <a href="<?php echo BASE_URI; ?>register.php"><?php _e(

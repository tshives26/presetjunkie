  ...
  switch ($_POST["action"]) {
    case "delete":
      ...
      if (!empty($_POST["batch"])) {
        $statement = $dbh->prepare(
          "DELETE FROM " . TABLE_LOG . " WHERE FIND_IN_SET(id, :delete)"
        );
        $params = [
          ":delete" => $delete_ids,
        ];
        ...
        $flash->success(
          __("The selected activities were deleted.", "cftp_admin")
        );
      } else {
    ...
    case "log_clear":
      ...
      $statement = $dbh->prepare(
        "DELETE FROM " .
          TABLE_LOG .
          " WHERE NOT ( FIND_IN_SET(action, :keep) ) "
      );
      $params = [
        ":keep" => $keep,
      ];
      ...
      $flash->success(
        __(
          "The log was cleared. Only data used for statistics remained. You can delete them manually if you want.",
          "cftp_admin"
        )
      );
      break;
...
<div class="row">
    ...
    if (isset($_GET["search"]) && !empty($_GET["search"])) {
      $cq .=
        " WHERE (owner_user LIKE :owner OR affected_file_name LIKE :file OR affected_account_name LIKE :account)";
      $next_clause = " AND";
        ...
        <div class="form_actions_left">
            <div class="form_actions_limit_results">
                ...
                <form action="actions-log.php" name="actions_filters" method="get" class="form-inline form_filters">
                    <?php form_add_existing_parameters(["activity"]); ?>
                    <div class="form-group group_float">
                        <label for="activity" class="sr-only"><?php _e(
                          "Filter activities",
                          "cftp_admin"
                        ); ?></label>
                            ...
                            <option value="all"><?php _e(
                              ...
                              "cftp_admin"
                                ...
                                foreach (
                                  ...
                                  as $action_number => $name
                                ) { ?>
                                        <option value="<?php echo $action_number; ?>" <?php if (
  isset($_GET["activity"]) &&
  $_GET["activity"] == $action_number
) {
        ...
        <form action="<?php echo $current_url; ?>" name="actions_list" method="post" class="form-inline batch_actions">
            ...
            while ($log = $sql->fetch()) {
              ...
              $tbody_cells = [
                ...
                [
                  "content" => !empty($this_action["part1"])
                    ? html_output($this_action["part1"])
                    : "",
                ],
                [
                ...
                [
                  "content" => !empty($this_action["part2"])
                    ? html_output($this_action["part2"])
                    : "",
                ],
                [
                  "content" => !empty($this_action["part3"])
                    ? html_output($this_action["part3"])
                    : "",
                ],
                [
                  "content" => !empty($this_action["part4"])
                    ? html_output($this_action["part4"])
                    : "",
                ],
              ];
